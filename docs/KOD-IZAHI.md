...
- **Tailwind CSS** (`@tailwindcss/vite` plugin ilə, `vite.config.js`-də) — **qısa layout utility-ləri** (`flex`, `items-center`, `justify-between`, `gap-3`, `text-center`, `cursor-pointer`, `truncate` və s.) üçün, CSS Modules-un ƏVƏZİNƏ YOX, ONUNLA BİRLİKDƏ istifadə olunur — bax Hissə 19-un sonu. Rənglər/radius/shadow kimi dizayn token-ləri YENƏ CSS dəyişənləri (`var(--color-green)` və s.) ilə, CSS Modules-da qalır — Tailwind YALNIZ tez-tez təkrarlanan qısa layout sətirlərini əvəz edir.
...
**`showIcon?: boolean`** — OPSİONAL PROP, DEFAULT `true`. Products/Campaigns/Categories-dəki SİLMƏ TƏSDİQLƏRİ heç bir dəyər VERMİR, ona görə DEFAULT `true` İLƏ, KÖHNƏ `delete-img.svg` İKONU HƏMİŞƏ GÖRÜNÜR (`{showIcon && <img src={deleteImg} .../>}`). `Sidebar.tsx`-DƏKİ ÇIXIŞ TƏSDİQİ İSƏ `showIcon={false}` ÖTÜRÜR — ÇÜNKİ ÇIXIŞ ETMƏK "SİLMƏ" DEYİL, İKONSUZ, YALNIZ MƏTN+DÜYMƏLƏRLƏ GÖSTƏRİLİR.
...
export default function Sidebar() {
  ...
  return (
    <aside className={`flex flex-col ${styles.aside}`}>
      ...
      <button onClick={() => setConfirmOpen(true)} className={`flex items-center gap-3 cursor-pointer text-left ${styles.logoutBtn}`}>
      ...
      <ConfirmModal
        ...
        showIcon={false}
        onConfirm={() => { setConfirmOpen(false); logout(); navigate('/login') }}
...
- `className={({ isActive }) => ...}` — `NavLink`-İN XÜSUSİ BİR XÜSUSİYYƏTİDİR: `className` PROP-UNA STRİNG ƏVƏZİNƏ BİR FUNKSİYA VERİLƏ BİLƏR, BU FUNKSİYA `{ isActive }` (CARİ URL BU LİNKƏ UYĞUNDURMU, `boolean`) ALIR VƏ UYĞUN KLASI QAYTARIR — react-router-dom-un ÖZ TİPLƏRİ `isActive`-İN `boolean` OLDUĞUNU AVTOMATİK TƏMİN EDİR. `flex items-center gap-3` KİMİ SÖZLƏR — YUXARIDA (Texnologiyalar) QEYD OLUNAN **Tailwind** UTILITY KLASLARIDIR, `styles.link`-İN YANINDA BİRLİKDƏ İŞLƏDİLİR.
- **ÇIXIŞ İNDİ BİRBAŞA `logout()` ÇAĞIRMIR** — `onClick={() => setConfirmOpen(true)}` sadəcə `confirmOpen` STATE-İNİ `true` EDİR, BU DA `ConfirmModal`-I AÇIR ("Hesabdan çıxmaq istədiyinizə əminsiniz?"). YALNIZ İSTİFADƏÇİ **Təsdiqlə** DÜYMƏSİNƏ BASSA, `onConfirm` İŞƏ DÜŞÜR: MODAL BAĞLANIR, SONRA `logout()` (ZUSTAND STORE-U TƏMİZLƏYİR), SONRA `navigate('/login')` (BROWSER-İ `/login`-Ə APARIR). **İndi yox** DÜYMƏSİ (`onCancel`) İSƏ SADƏCƏ MODALI BAĞLAYIR, HEÇ NƏ BAŞ VERMİR. BU DƏYİŞİKLİK ÖZÜ BİR REAL UX QƏRARIDIR — SƏHVƏN ÇIXIŞA KLİK ETMƏK ARTIQ DƏRHAL SESSİYANI BİTİRMİR.
...
**STATUS DROPDOWN-UNUN RƏNGİ — bir neçə cəhd sonra tapılan düzgün həll:**
```tsx
// ORDER_STATUS_BADGE_COLOR dəyərləri BadgeColor açarlarıdır ('amber' kimi), ƏSL CSS rəngi DEYİL —
// 'amber' KEÇƏRLİ bir CSS rəng adı DEYİL, ona görə birbaşa style={{color}}-a verilə bilməz.
// Badge.module.css-in istifadə etdiyi EYNİ CSS dəyişənlərinə map edirik.
...
>
  {ORDER_STATUS_OPTIONS.map((s) => (
    // option-lar select-in rəngini MİRAS ALIR (CSS-in inheritance qaydası) —
    // bunsuz, dropdown AÇILANDA bütün seçimlər (Gözləyir, Hazırdır, HAMISI)
    // cari statusun rənginə boyanardı. Hər option-a AYRICA neytral rəng verib bunu kəsirik.
...
.statusSelect {
  ...
  border: 1px solid currentColor; /* HƏMİŞƏ mətnin öz rənginə bərabər — fokusdan asılı deyil */
  padding: 4px 8px;
  font-size: 14px;
  color: var(--color-text);
  outline: none; /* brauzerin default qalın focus outline-ını söndürür */
}
```
Bu, DÖRD AYRI TƏLƏBİN nəticəsidir (hamısı birlikdə OXUNMALIDIR):
1. **Bağlı dropdown status rənginə uyğun görünsün** (məs. "Ləğv edildi" → qırmızı) — `style={{ color: STATUS_TEXT_COLOR[...] }}` `<select>`-in ÖZÜNƏ.
2. **AÇIQ dropdown siyahısındakı bütün seçimlər neytral (ağ fonda qara) qalsın**, cari statusun rənginə boyanmasın — `color` CSS-də VALİDEYNDƏN UŞAĞA (`<select>`-dən `<option>`-lara) MİRAS QALIR, ona görə hər `<option>`-a AYRICA `style={{ color: 'var(--color-text)' }}` verilib, bu mirası "kəsir".
3. **Border rəngi HƏMİŞƏ mətn rənginə bərabər olsun**, təkcə fokus zamanı yox — `border: 1px solid currentColor` (`currentColor` — CSS-in ÖZ AÇAR SÖZÜDÜR, "bu elementin ÖZ `color` dəyəri" deməkdir, əlavə JS/dəyişən lazım deyil).
4. **Kənara klik edəndə (blur) border rəngi dəyişməsin** — `outline: none` brauzerin default qalın focus halqasını söndürür, VƏ border-color HƏMİŞƏ (fokuslu/fokussuz fərq etmədən) eyni `currentColor` qaydasına tabedir, ona görə ayrıca `:focus` qaydası artıq lazım deyil.
...
**CSS Modules + Tailwind — ikisi BİRLİKDƏ, iki AYRI VƏZİFƏ ilə:** Layihəyə sonradan **Tailwind CSS** (`@tailwindcss/vite` plugin, `vite.config.js`-də; `@import "tailwindcss";` isə `src/index.css`-in ilk sətrində) əlavə olunub, AMMA CSS Modules FAYLLARI SİLİNMƏYİB — İKİSİ BİRLİKDƏ İŞLƏYİR:
```tsx
// Sidebar.tsx-dən nümunə
<aside className={`flex flex-col ${styles.aside}`}>
```
`flex flex-col` — Tailwind-in HAZIR utility klasları (`display: flex; flex-direction: column;` DEMƏKDİR), `styles.aside` İSƏ HƏMİN KOMPONENTİN ÖZ `Sidebar.module.css`-İNDƏN GƏLİR (EN, PADDING, BACKGROUND VƏ S.). **Qayda sadədir:** `display`/`flex-direction`/`align-items`/`justify-content`/`flex-wrap`/`gap`/`text-align`/`cursor`/`resize`/`overflow-hidden+ellipsis+nowrap` (`truncate`) KİMİ QISA, TEZ-TEZ TƏKRARLANAN sətirlər Tailwind class-larına KEÇİRİLİB VƏ CSS Module fayllarından ÇIXARILIB; RƏNGLƏR, `border-radius`, `box-shadow`, ÖLÇÜLƏR İSƏ HƏLƏ DƏ `.module.css`-DƏ, CSS DƏYİŞƏNLƏRİ (`var(--color-green)`) İLƏ QALIR — ÇÜNKİ BUNLAR LAYİHƏNİN "DİZAYN TOKEN-LƏRİ" SİSTEMİDİR (BİR YERDƏ TƏYİN OLUNUB, HƏR YERDƏ İSTİFADƏ OLUNUR) VƏ TAILWIND-Ə KÖÇÜRÜLMƏSİ BU TƏK-MƏNBƏ QAYDASINI POZARDI.
...
```
- `plugins: [react(), tailwindcss()]` — `react()` JSX-i BROWSER-İN ANLADIĞI JS-Ə ÇEVİRİR (Hissə 2-DƏKİ JSX-Ə BAXIN), `tailwindcss()` İSƏ Hissə 19-DA İZAH OLUNAN **Tailwind CSS**-İ AKTİV EDİR — BU PLUGIN OLMASA, `src/index.css`-DƏKİ `@import "tailwindcss";` SƏTRİ HEÇ NƏ ETMƏZ VƏ `flex`, `gap-3` KİMİ KLASLAR TANINMAZDI.
- `resolve.alias: { '@': ... }` — Hissə 2-DƏKİ `@/` QISAYOLUNUN VİTE ÜÇÜN TƏYİNATI (`tsconfig.json`-DAKI EYNİ QISAYOLLA SİNXRON SAXLANMALIDIR, YUXARIDA QEYD OLUNDU).
- `manualChunks(id)` — BUILD ZAMANI, `react`/`react-dom`/`react-router-dom`/`@tanstack`/`axios`/`zustand`/`sonner` KİMİ "XARİCİ" PAKETLƏRİ TƏTBİQİN ÖZ KODUNDAN AYRI, TƏK BİR `vendor` FAYLINA YIĞIR — BELƏLİKLƏ, TƏTBİQ KODU DƏYİŞƏNDƏ (YENİ SƏHİFƏ, DÜZƏLİŞ VƏ S.), BRAUZER ARTIQ DƏYİŞMƏYƏN `vendor` FAYLINI TƏKRAR YÜKLƏMİR (CACHE-DƏN İSTİFADƏ EDİR).
...
| **Tailwind CSS** | Hazır, qısa utility klasları (`flex`, `gap-3`, `text-center`) ilə CSS yazmaq üsulu — bu layihədə CSS Modules-la BİRLİKDƏ, yalnız qısa layout sətirləri üçün istifadə olunur |
| **`currentColor`** | CSS-in açar sözü — "bu elementin ÖZ `color` dəyəri", `border`/`fill` kimi başqa xassələrdə istifadə edilə bilər |
